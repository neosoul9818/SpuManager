...
<body>
 属性列表页面<br>
 	...
 	<c:forEach items="${attr_list }" var="attr" varStatus="status">
 		<input type="checkbox" onclick="show_check(${attr.id},this.checked)" value="${attr.id }" name="list_av[${status.index}].shxm_id"/> ${attr.shxm_mch}
 	</c:forEach>
 	<hr>
	<c:forEach items="${attr_list }" var="attr" varStatus="status">
	 	<div id="attr_radio_${attr.id }" style="display: none">
			 ${attr.shxm_mch}:
